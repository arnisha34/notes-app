...
export const NoteTags = () => {
  ...
  return(
    <div className="flex items-center gap-2">
      ...
      <div className="flex flex-wrap gap-2">{ctx.tags?.map((tag, id) => <span key={id} className="bg-neutral-100 flex gap-2 items-center px-2 py-1 rounded-md dark:bg-slate-800">{tag} <HiXMark className="cursor-pointer w-3" onClick={() => {ctx.setTags(tag.toLowerCase()); deleteTag(tag);}}/></span>)}</div>
    </div>
  )
}