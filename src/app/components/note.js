    ...
    <div className="w-full mx-auto">
      <div id="main-section" className="flex h-screen">
        <section className="border-r-1 border-gray-400 flex flex-col p-5 w-xs">
          ...
          <div className={`${ctx.newNote === "addNote" || addNote.length ?"hidden":""} bg-neutral-100 flex flex-col gap-3 items-center p-3 rounded-lg text-sm dark:bg-slate-800 dark:text-slate-400"}`}><PencilSquareIcon className="text-gray-500 w-8 dark:text-slate-400"/>You haven&apos;t added any notes yet. Begin a new note to jot down your thoughts and ideas.</div>
          <SavedNotes />
        </section>
        ...
        <>
          ...
          <div className="note-text h-3/4">
            <textarea placeholder="Start typing here..." value={noteText} className="focus:outline-none w-full h-full border-t-1 border-gray-400 py-5 resize-none font-medium" onChange={(e) => setNoteText(e.target.value)}>[noteText]</textarea>
          </div>
